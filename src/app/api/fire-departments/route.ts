...
export async function GET(request: NextRequest) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.json({ error: 'Failed to fetch fire departments', details: error.message }, { status: 500 });
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!name || typeof name !== 'string') {
            return NextResponse.json({ success: false, message: 'Fire station name is required' }, { status: 400 });
        }
        const { data, error } = await supabase
            .from('fire_departments')
            .insert({
        ...
        if (error) {
            ...
            return NextResponse.json({ success: false, message: 'Failed to add fire station', details: error.message }, { status: 500 });
        }
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!id) {
            return NextResponse.json({ success: false, message: 'Fire station ID is required' }, { status: 400 });
        }
        const body = await request.json();
        const { name, city, county, address, latitude, longitude } = body;
        ...
        if (county !== undefined)
            updateData.county = county?.trim() || null;
        ...
        if (error) {
            ...
            return NextResponse.json({ success: false, message: 'Failed to update fire station', details: error.message }, { status: 500 });
        }
...
export async function DELETE(request: NextRequest) {
    try {
        ...
        if (error) {
            ...
            return NextResponse.json({ success: false, message: 'Failed to delete fire station', details: error.message }, { status: 500 });
        }
