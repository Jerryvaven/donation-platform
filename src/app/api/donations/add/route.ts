...
export async function POST(request: NextRequest) {
    try {
        ...
        const { donorName, city, state, address, productId, quantity, matched, fireDepartmentId, } = body;
        if (!donorName || !productId || !quantity) {
            return NextResponse.json({ error: "Donor name, product, and quantity are required" }, { status: 400 });
        }
        ...
        if (existingDonor) {
            ...
            const newTotalValue = parseFloat(existingDonor.total_donated_value.toString()) + productValue;
            const newTotalProducts = (existingDonor.total_products_donated || 0) + parseInt(quantity);
            await supabase
                .from("donors")
                .update({
        ...
        const { data: donation, error: donationError } = await supabase
            ...
            .insert([
            {
                ...
                fire_department_id: matched && fireDepartmentId ? fireDepartmentId : null,
