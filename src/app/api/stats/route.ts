...
export async function GET(request: NextRequest) {
    try {
      ...
      `);
        ...
        const saunaDonated = productDonations?.filter((d: any) => d.products?.category === 'Sauna').reduce((sum: number, d: any) => sum + d.quantity, 0) || 0;
        const coldPlungeDonated = productDonations?.filter((d: any) => d.products?.category === 'Cold Plunge').reduce((sum: number, d: any) => sum + d.quantity, 0) || 0;
        const fireDepartmentsReached = new Set(productDonations
            ?.filter((d: any) => d.fire_department_id)
            .map((d: any) => d.fire_department_id)).size;
        ...
        const newDonorsToday = donors?.filter((d: any) => new Date(d.created_at).toISOString().split('T')[0] === today.toISOString().split('T')[0]).length || 0;
        ...
        const lastMonthFireDepartmentsReached = new Set(lastMonthDonations
            ?.filter((d: any) => d.fire_department_id)
            .map((d: any) => d.fire_department_id)).size;
