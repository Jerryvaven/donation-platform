...
export default function AddFireStationModal({ showAddFireStationModal, setShowAddFireStationModal, onFireStationAdded, onFireStationUpdated, editStation, darkMode = false }: AddFireStationModalProps) {
    ...
    return (<AnimatePresence>
      {showAddFireStationModal && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className={`${darkMode ? 'bg-[#1E1E1E]' : 'bg-white'} rounded-2xl max-w-lg w-full p-8 shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide`} style={{
            ...
            <div className="flex justify-between items-center mb-8">
              <div className="flex items-center gap-4">
                <motion.div className={`p-3 ${darkMode ? 'bg-[#EF4444]' : 'bg-red-600'} rounded-xl`} whileHover={{ rotate: 360 }} transition={{ duration: 0.6 }}>
              ...
              <motion.button onClick={() => setShowAddFireStationModal(false)} whileHover={{ scale: 1.1, rotate: 90 }} whileTap={{ scale: 0.9 }} className={`p-2 rounded-lg transition-all ${darkMode ? 'text-[#808080] hover:text-white hover:bg-[#242424]' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-100'}`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <input type="text" id="stationName" value={newStationName} onChange={(e) => setNewStationName(e.target.value)} required className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all`} placeholder="e.g., Newport Beach Fire Department"/>
              </div>
              ...
              <div>
                ...
                <input type="text" id="stationCity" value={newStationCity} onChange={(e) => setNewStationCity(e.target.value)} className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all`} placeholder="e.g., Newport Beach"/>
              </div>
              ...
              <div>
                ...
                <input type="text" id="stationCounty" value={newStationCounty} onChange={(e) => setNewStationCounty(e.target.value)} className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all`} placeholder="e.g., Orange County"/>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <textarea id="stationAddress" value={newStationAddress} onChange={(e) => setNewStationAddress(e.target.value)} rows={3} className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all resize-none`} placeholder="Full address of the fire station..."/>
                  <motion.button type="button" onClick={handleFetchCoordinates} disabled={fetchingCoords || !newStationAddress.trim()} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${darkMode ? 'bg-[#333333] text-[#B3B3B3] hover:bg-[#444444] hover:text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'} disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 text-sm`}>
                    {fetchingCoords ? (<>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" id="stationLatitude" value={newStationLatitude} onChange={(e) => setNewStationLatitude(e.target.value)} className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all`} placeholder="e.g., 33.6189"/>
                </div>
                ...
                <div>
                  ...
                  <input type="text" id="stationLongitude" value={newStationLongitude} onChange={(e) => setNewStationLongitude(e.target.value)} className={`w-full px-4 py-2.5 ${darkMode ? 'bg-[#242424] border-[#333333] text-white' : 'bg-white border-gray-300 text-gray-900'} border rounded-lg focus:ring-2 ${darkMode ? 'focus:ring-[#EF4444] focus:border-[#EF4444]' : 'focus:ring-red-600 focus:border-red-600'} transition-all`} placeholder="e.g., -117.9298"/>
                </div>
              </div>

              <div className="flex gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
                <motion.button type="button" onClick={() => setShowAddFireStationModal(false)} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={`flex-1 py-3 px-6 rounded-lg font-semibold transition-all duration-200 ${darkMode ? 'bg-[#242424] text-[#B3B3B3] hover:bg-[#333333] hover:text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Cancel
                </motion.button>
                <motion.button type="submit" disabled={addingStation} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className={`flex-1 py-3 px-6 rounded-lg font-semibold transition-all duration-200 ${darkMode ? 'bg-[#EF4444] text-white hover:bg-[#DC2626]' : 'bg-red-600 text-white hover:bg-red-700'} disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2`}>
                  {addingStation ? (<>
                      <FaSync className="animate-spin"/>
