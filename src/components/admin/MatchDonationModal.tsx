...
export default function MatchDonationModal({
  showModal,
  setShowModal,
  donation,
  onDataRefresh,
  darkMode = false
}: MatchDonationModalProps) {
  ...
  const filteredFireDepartments = fireDepartments.filter(fd =>
    fd.name.toLowerCase().includes(fireDepartmentSearch.toLowerCase()) ||
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!donation || !selectedFireDepartment) return
  ...
  if (!showModal || !donation) return null
        ...
        {showModal && (
          ...
          >
            <motion.div
              ...
              className={`${darkMode ? 'bg-[#1E1E1E]' : 'bg-white'} rounded-2xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto scrollbar-hide`}
              style={{
                scrollbarWidth: 'none',
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className={`p-4 rounded-lg border ${
              darkMode ? 'bg-[#1A1A1A] border-[#333333]' : 'bg-gray-50 border-gray-200'
            }`}>
            ...
            <div>
              ...
              <div className={`max-h-40 overflow-y-auto border rounded-lg ${
                darkMode ? 'border-[#333333]' : 'border-gray-300'
              }`}>
                {loading ? (
                  <div className={`flex items-center justify-center py-4 ${darkMode ? 'text-[#B3B3B3]' : 'text-gray-500'}`}>
                    <FaSync className="animate-spin mr-2" />
                    Loading...
                  </div>
                ) : filteredFireDepartments.length === 0 ? (
                  <div className={`text-center py-4 text-sm ${darkMode ? 'text-[#808080]' : 'text-gray-500'}`}>
                    No fire departments found
                  </div>
                ) : (
                  filteredFireDepartments.map((fd) => (
                    <motion.div
                      key={fd.id}
                      onClick={() => setSelectedFireDepartment(fd.id)}
                      whileHover={{ backgroundColor: darkMode ? '#333333' : '#F3F4F6' }}
                      className={`p-3 cursor-pointer border-b last:border-b-0 ${
                        selectedFireDepartment === fd.id
            ...
            <div className="flex gap-3">
              ...
              <motion.button
                type="submit"
                disabled={!selectedFireDepartment || saving}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className={`flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-700`}
              >
                {saving ? (
