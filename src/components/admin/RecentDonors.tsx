...
export default function RecentDonors({ recentDonors, loading, onDataRefresh, onViewDonation, onEditDonation, darkMode }: RecentDonorsProps) {
    ...
    return (<motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.8 }} className={`lg:col-span-3 rounded-xl p-6 shadow-sm border ${darkMode ? 'bg-[#242424] border-[#333333]' : 'bg-white border-gray-100'}`}>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div className="relative w-full sm:w-auto">
          ...
          <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search donors..." className={`border rounded-lg pl-10 pr-4 py-2 focus:ring-2 focus:outline-none w-full sm:w-64 transition-all ${darkMode
      ...
      <AnimatePresence>
        {message && (<motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className={`mb-4 p-4 rounded-lg flex items-center gap-3 ${message.type === 'success'
            ...
            {message.type === 'success' ? (<FaCheckCircle className={`text-xl flex-shrink-0 ${darkMode ? 'text-[#22C55E]' : 'text-green-600'}`}/>) : (<FaTimes className={`text-xl flex-shrink-0 ${darkMode ? 'text-[#EF4444]' : 'text-red-600'}`}/>)}
      ...
      {loading ? (<div className={`text-center py-12 flex items-center justify-center gap-3 ${darkMode ? 'text-[#B3B3B3]' : 'text-gray-600'}`}>
          <FaSync className={`animate-spin text-xl ${darkMode ? 'text-[#3B82F6]' : 'text-black'}`}/>
          <span>Loading donors...</span>
        </div>) : recentDonors.length === 0 ? (<div className={`text-center py-8 ${darkMode ? 'text-[#B3B3B3]' : 'text-gray-600'}`}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                ...
                .map((donor, index) => (<motion.tr key={donor.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ delay: index * 0.05, duration: 0.3 }} className={`border-b transition-colors duration-200 cursor-pointer ${darkMode ? 'border-[#333333] hover:bg-[#1E1E1E]' : 'border-gray-100 hover:bg-gray-50'}`}>
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-3">
                            <motion.div whileHover={{ scale: 1.1 }} className="w-10 h-10 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                              {donor.productImage ? (<img src={donor.productImage} alt={donor.productName} className="w-full h-full object-cover" onError={(e) => {
                        e.currentTarget.style.display = 'none';
                        e.currentTarget.nextElementSibling?.classList.remove('hidden');
                    }}/>) : null}
                              <div className={`w-full h-full flex items-center justify-center font-semibold ${donor.productImage ? 'hidden' : ''} ${darkMode ? 'bg-[#3B82F6] text-white' : 'bg-gray-900 text-white'}`}>
                    ...
                    year: 'numeric'
                        ...
                        <td className="py-4 px-4 text-center">
                          <div className="flex items-center gap-2">
                            <motion.span whileHover={{ scale: 1.05 }} className={`px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1 w-fit ${donor.status === 'MATCHED'
                    ...
                    : (darkMode ? 'bg-[#808080]/20 text-[#B3B3B3]' : 'bg-gray-100 text-gray-700')}`}>
                            ...
                            {donor.status === 'PENDING' && (<motion.button onClick={() => handleMatchDonation(donor)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`px-2 py-1 text-xs font-semibold rounded transition-colors duration-200 ${darkMode ? 'bg-[#3B82F6]/20 hover:bg-[#3B82F6]/30 text-[#3B82F6]' : 'bg-blue-100 hover:bg-blue-200 text-blue-700'}`}>
                                Match
                              </motion.button>)}
                          </div>
                        </td>
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-2">
                            <motion.button onClick={() => onViewDonation?.(donor)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className={`p-2 transition-colors duration-200 ${darkMode ? 'text-[#808080] hover:text-[#3B82F6]' : 'text-gray-400 hover:text-blue-600'}`} title="View">
                              <FaEye />
                            </motion.button>
                            <motion.button onClick={() => onEditDonation?.(donor)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className={`p-2 transition-colors duration-200 ${darkMode ? 'text-[#808080] hover:text-[#FACC15]' : 'text-gray-400 hover:text-yellow-600'}`} title="Edit">
                              <FaEdit />
                            </motion.button>
                            <motion.button onClick={() => handleDeleteClick(donor)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className={`p-2 transition-colors duration-200 ${darkMode ? 'text-[#808080] hover:text-[#EF4444]' : 'text-gray-400 hover:text-red-600'}`} title="Delete">
                              <FaTrash />
                            </motion.button>
                          </div>
                        </td>
          ...
          {(() => {
                const filteredDonors = recentDonors.filter(donor => searchQuery === '' ||
                    donor.donorName.toLowerCase().includes(searchQuery.toLowerCase()) ||
                ...
                {totalPages > 1 && (<div className="flex gap-1">
                    <motion.button onClick={() => setCurrentPage(prev => Math.max(1, prev - 1))} disabled={currentPage === 1} whileHover={currentPage !== 1 ? { scale: 1.05 } : {}} whileTap={currentPage !== 1 ? { scale: 0.95 } : {}} className={`px-3 py-1 rounded text-sm transition-colors ${currentPage === 1
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (<motion.button key={page} onClick={() => setCurrentPage(page)} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className={`px-3 py-1 rounded text-sm font-semibold transition-colors ${currentPage === page
                                ? (darkMode ? 'bg-[#3B82F6] text-white' : 'bg-black text-white')
                                : (darkMode ? 'bg-[#242424] text-[#B3B3B3] hover:bg-[#333333]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}`}>
                        {page}
                      </motion.button>))}
                    <motion.button onClick={() => setCurrentPage(prev => Math.min(totalPages, prev + 1))} disabled={currentPage === totalPages} whileHover={currentPage !== totalPages ? { scale: 1.05 } : {}} whileTap={currentPage !== totalPages ? { scale: 0.95 } : {}} className={`px-3 py-1 rounded text-sm transition-colors ${currentPage === totalPages
                            ? (darkMode ? 'bg-[#1E1E1E] text-[#808080] cursor-not-allowed' : 'bg-gray-50 text-gray-400 cursor-not-allowed')
                            : (darkMode ? 'bg-[#242424] text-[#B3B3B3] hover:bg-[#333333]' : 'bg-gray-100 text-gray-600 hover:bg-gray-200')}`}>
                      Next
                    </motion.button>
      ...
      <DeleteConfirmModal isOpen={showDeleteModal} onClose={() => {
            setShowDeleteModal(false);
            setDonationToDelete(null);
        }} onConfirm={handleDeleteConfirm} itemName={donationToDelete?.donorName} itemValue={`$${donationToDelete?.productValue.toLocaleString('en-US', { minimumFractionDigits: 0 })}`} isDeleting={isDeleting} darkMode={darkMode}/>

      <MatchDonationModal showModal={showMatchModal} setShowModal={setShowMatchModal} donation={donationToMatch} onDataRefresh={onDataRefresh} darkMode={darkMode}/>
    </motion.div>);
}
