...
export default function DonorTrends({ monthlyData, darkMode }: DonorTrendsProps) {
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.6 }} className={`rounded-xl p-6 shadow-sm border mb-8 ${darkMode ? 'bg-[#242424] border-[#333333]' : 'bg-white border-gray-100'}`}>
      ...
      <div className="flex items-end justify-between h-64 gap-2">
        {monthlyData.slice(0, 11).map((data, index) => {
            ...
            const matchedHeight = maxValue > 0
                ...
                : 0;
            return (<div key={data.month} className="flex-1 flex flex-col items-center gap-1">
              <div className="w-full flex gap-1 items-end" style={{ height: '256px' }}>
                <motion.div initial={{ height: 0 }} animate={{ height: donationHeight }} transition={{ duration: 0.8, delay: 0.7 + index * 0.05 }} className={`flex-1 rounded-t cursor-pointer relative group ${darkMode ? 'bg-[#3B82F6]' : 'bg-black'}`}>
                  {data.donations > 0 && (<div className={`absolute -top-8 left-1/2 transform -translate-x-1/2 px-2 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 ${darkMode ? 'bg-[#1E1E1E] text-white' : 'bg-gray-900 text-white'}`}>
                      {formatCurrency(data.donations, 0)}
                    </div>)}
                </motion.div>
                <motion.div initial={{ height: 0 }} animate={{ height: matchedHeight }} transition={{ duration: 0.8, delay: 0.7 + index * 0.05 }} className={`flex-1 rounded-t cursor-pointer relative group ${darkMode ? 'bg-[#808080]' : 'bg-gray-300'}`}>
                  {data.matched > 0 && (<div className={`absolute -top-8 left-1/2 transform -translate-x-1/2 px-2 py-1 rounded text-xs whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 ${darkMode ? 'bg-[#1E1E1E] text-white' : 'bg-gray-700 text-white'}`}>
