...
const Products: React.FC<{
    ...
    darkMode?: boolean;
}> = ({ onProductClick, darkMode = false }) => {
    const [products, setProducts] = useState<Product[]>([]);
    const [loading, setLoading] = useState(true);
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                let response = await fetch('/api/ecommerce-products?category=Cold%20Plunge');
                let data = await response.json();
                if (!data.data || data.data.length === 0 || data.error) {
                    console.log('Fetching from regular products table as fallback');
                    response = await fetch('/api/products');
                    data = await response.json();
                    const coldPlungeProducts = (data.data || [])
                        .filter((p: DbProduct) => p.category === 'Cold Plunge')
                        .map((p: DbProduct) => ({
                        name: p.name,
                        price: `$${(p.value || 0).toLocaleString()}`,
                        image: p.image_url || '/assets/coldplunges/product/default.png',
                        benefits: p.features || [
                            p.description || 'Premium cold plunge therapy',
                    ...
                    const coldPlungeProducts = (data.data || []).map((p: DbProduct) => ({
                        name: p.name,
                        price: `$${(p.price || 0).toLocaleString()}`,
                        image: p.image_url || '/assets/coldplunges/product/default.png',
                        benefits: p.card_features || p.features || [
                            p.description || 'Premium quality cold plunge',
                            'Precision temperature control',
                            'Durable construction',
                            'Warranty included'
                        ],
    ...
    if (products.length === 0) {
        ...
        <p className={darkMode ? 'text-gray-400' : 'text-gray-600'}>No cold plunge products available at the moment.</p>
      </div>);
    }
