...
export default function DonorListItem({ donor, index, darkMode }: DonorListItemProps) {
    ...
    return (<li className={`px-6 py-4 transition-all duration-200 cursor-pointer ${darkMode ? 'hover:bg-[rgba(255,255,255,0.08)]' : 'hover:bg-gray-50'} ${index < 3 ? (darkMode ? 'bg-[rgba(255,255,255,0.04)]' : 'bg-gradient-to-r from-yellow-50 to-transparent') : ''}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center">
          <div className="flex-shrink-0">
            <span className={`inline-flex items-center justify-center h-10 w-10 rounded-full text-sm font-bold ${index === 0 ? 'bg-yellow-400 text-yellow-900' :
