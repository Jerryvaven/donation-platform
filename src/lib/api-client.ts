...
export async function fetchEcommerceProducts(category?: 'Sauna' | 'Cold Plunge') {
    const url = category
        ? `${API_BASE}/ecommerce-products?category=${category}`
        : `${API_BASE}/ecommerce-products`;
    const response = await fetch(url);
    if (!response.ok) {
        throw new Error('Failed to fetch ecommerce products');
    }
    return response.json();
}
export async function addEcommerceProduct(data: {
    name: string;
    ...
    const response = await fetch(`${API_BASE}/ecommerce-products`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to add ecommerce product');
    }
    return response.json();
}
export async function updateEcommerceProduct(id: string, data: {
    name?: string;
    category?: 'Sauna' | 'Cold Plunge';
    price?: number;
    description?: string;
    ...
    const response = await fetch(`${API_BASE}/ecommerce-products?id=${id}`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
    ...
    if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to update ecommerce product');
    }
    return response.json();
}
export async function deleteEcommerceProduct(id: string) {
    ...
    if (!response.ok) {
        const error = await response.json();
        throw new Error(error.error || 'Failed to delete ecommerce product');
    }
    return response.json();
}
